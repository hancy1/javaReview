package chap17_네트워크.한채영.uni.chap01_inet.sample;
...
public class InetSample {
	...
	public void isSample() {
		...
		try {
			...
			System.out.println("내 pc명: " + localIp.getHostName()); //호스트의 이름
			System.out.println("내 IP: " + localIp.getHostAddress()); //Ip주소값
		
		InetAddress naverIp = InetAddress.getByName("www.naver.com"); //도메인주소를 써서 호스트 명과 주소를 받아올 수도 있음
		System.out.println("네이버 서버명: " + naverIp.getHostName()); //호스트의 이름
		System.out.println("네이버 서버 주소: " + naverIp.getHostAddress()); //Ip주소값
		} catch (UnknownHostException e) { // 연결 시에 host가 없을 경우 오류가 생기기 때문에 예외처리를 해줘야한다
			e.printStackTrace();
		}
